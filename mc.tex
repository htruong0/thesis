...
\chapter{Monte Carlo Event Generators}\label{chap:MC}
    ...
    in $d$-dimensional parameter space points $\vec{x}$
    ...
    \end{equation}
    where the points $\vec{x}$ are randomly,
    ...
    was explored in Reference \cite{Danziger:2021eeg} wherein a
    ...
    This is motivated by the factorisation theorem (Section~\ref{sec:factorisation})
    ...
    one-loop level required for SM predictions, see for instance Reference \cite{Campbell:2017hsr}.
    ...
    of an event generator follow phenomenological, non-perturbative models.
    ...
    The reasoning behind this approach is that $\delta$
