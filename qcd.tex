...
\section{Antenna functions}\label{sec:antenna_functions}
    ...
    processes. A pictorial representation is also given in Figure~\ref{fig:antenna}.
    ...
    \begin{figure}
        \includegraphics[width=\linewidth]{qcd/antenna.pdf}
    ...
    d \bar{d}$ + $ng$.
    For this we require the $A$, $D$, and $F$ antenna functions,
    see Table~\ref{table:antenna_functions}.
    The $i,j,k$ indices will be fixed such that $d = 1$, $\bar{d} = 2$,
    and $g \in \{3, 4, 5\}$.
    ...
    from the $\mathcal{P}oles$ operator coming from
    the IR singularity operators (cite here). Therefore,
    care has to be taken to extract all the finite
    remainders from the antenna functions.
    ...
    are subleading. They become important when
    building subtraction terms to exactly cancel divergences
    but we do not require this level of accuracy for
    our applications.

    The antenna functions have some functions in common,
    namely the singularity operators (cite here)
    ...
    The tree-level three-parton antenna function is given by
    ...
    \end{equation}
    where the IR structure is apparent. The first two terms
    correspond to the gluon going collinear with the quark
    and antiquark, where as the final term accounts for the soft
    limit.

    \subsubsection{Three-parton one-loop antenna function}
    The leading colour one-loop antenna functions are
    given by
    \begin{align}\label{eqn:A31}
        \mathcal{P}oles\left(A_{3}^{1}(1_{q}, 3_{g}, 2_{\bar{q}})\right) =& \; 2\left(\mathbf{I}_{qg}^{(1)}(\epsilon, s_{13}) + \mathbf{I}_{qg}^{(1)}(\epsilon, s_{23}) - \mathbf{I}_{q\bar{q}}^{(1)}(\epsilon,s_{123})\right)A_{3}^{0}(1_{q},3_{g},2_{\bar{q}}) \, , \\
        ...
        &+\dfrac{1}{s_{123}} + \dfrac{s_{12}+s_{23}}{2s_{123}s_{13}} + \dfrac{s_{12}+s_{13}}{2s_{123}s_{23}} \nonumber \\
        &-\dfrac{s_{13}}{2s_{123}(s_{12}+s_{13})}-\dfrac{s_{23}}{2s_{123}(s_{12}+s_{23})} \nonumber \\
        &+\dfrac{\log{y_{13}}}{s_{123}}\left(2-\dfrac{1}{2}\dfrac{s_{13}s_{23}}{(s_{12}+s_{23})^{2}} + 2\dfrac{s_{13}-s_{23}}{s_{12}+s_{23}}\right) \nonumber  \\
        &+\dfrac{\log{y_{23}}}{s_{123}}\left(2-\dfrac{1}{2}\dfrac{s_{13}s_{23}}{(s_{12}+s_{13})^{2}} + 2\dfrac{s_{23}-s_{13}}{s_{12}+s_{13}}\right) \, , 
    \end{align}
