...
\section{Introduction to quantum chromodynamics}
    ...
    is dependent on the energy scale of the process, $\mu$. This
...
\section{Factorisation theorem}
    ...
    This fluctuation timescale is much longer
    than the timescale of the interaction between
    an incoming probing parton from another proton in a
    highly energetic collision, like at the LHC.
    ...
    distribution of partons within a parton
    ...
    \end{equation}
    where $a$ is the parton identified from hadron $A$,
    and $b$ is the parton identified from hadron $B$.
    ...
    the soft effects in a scattering process below energy
    ...
    with the scattering occurs at energy scale $Q$.
    ...
    The discussion of how $\hat{\sigma}_{ab \rightarrow n}$
    is computed is discussed in detail in the next section.

\section{Scattering amplitudes and cross-sections}
    {\color{red} Cross-sections here are all
    partonic cross-sections. Need to talk about
    how they can be calculated order by order
    in perturbation theory with Feynman diagrams.}
